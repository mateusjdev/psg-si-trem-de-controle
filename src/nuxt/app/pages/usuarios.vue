      ...
      <div class="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
        ...
        <div class="flex h-full flex-col justify-end-safe gap-4">
          <h1 class="mb-2 text-2xl">
            {{ editando ? "Editando usuário" : "Novo usuário" }}
          </h1>
        ...
        <div class="flex h-full flex-col justify-center">
          <h2 class="mb-4 flex flex-col items-center text-xl font-semibold">
            Permissões
          </h2>
          <div class="flex flex-col gap-1">
            ...
            <p class="text-sm">
              Permite apenas visualizar informações de produtos.
            </p>
          </div>
        </div>
        <!--Coluna 4-->
...
async function validarFotoPerfil(event: Event) {
  ...
  if (
    ...
    imageFileToBase64(target.files[0])
      ...
      .catch((message) =>
        notificacoes.addNotification(message, { isError: true }),
      );
  }
}
...
async function editar(usuario: GetUsuarioDto) {
  ...
  if (res.ok && res.responseBody) {
    ...
    else if (res.responseBody.includes(Permissoes.Consulta))
...
async function confirmar() {
  if (!editando.value) {
    ...
    const res2 = await apiPermissoes.definir(res.responseBody, [
      permissoes.value,
    ]);
    if (res2.ok) {
      notificacoes.addNotification(
        "Usuário e permissões configurados com sucesso.",
      );
      limparCampos();
      obterUsuarios();
    }
    ...
    const res2 = await apiUsuario.alterarSenha(
      usuarioId.value!,
      usuarioSenha.value,
    );
    if (!res2.ok) return;
    // TODO: Atualizar senha opcional?
    const res3 = await apiPermissoes.definir(usuarioId.value!, [
      permissoes.value,
    ]);
    if (!res3.ok) return;
    notificacoes.addNotification(
